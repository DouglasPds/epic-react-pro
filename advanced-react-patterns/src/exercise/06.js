...
const useControlledSwitchWarning = (
  ...
  React.useEffect(() => {
    warning(
      ...
      Decide between using a controlled or uncontrolled \`${componentName}\` for the 
...
const useOnChangeReadyOnlyWarning = (
  controlPropValue,
  controlPropName,
  componentName,
  hasOnChange,
  ...
  React.useEffect(() => {
    warning(
      ...
      `A \`${controlPropName}\` prop was provided to \`${componentName}\` without 
      an \`${onChangeProp}\` handler. This will result in a read-only \`${controlPropName}\` 
      value. If you want it to be mutable, use \`${initialValueProp}\`. Otherwise, set either 
